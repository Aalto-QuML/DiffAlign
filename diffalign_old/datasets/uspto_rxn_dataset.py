...
atom_types = ['none', 'O', 'C', 'N', 'I', 'Cl', 'Si', 'F', 'Br', 'N+1', 'O-1', 'S', 'B', 'N-1', 
              'Zn+1', 'Cu', 'Sn', 'P+1', 'Mg+1', 'C-1', 'P', 'S+1', 'S-1', 'Se', 'Zn', 'Mg', 
              'U', 'Au', 'SuNo']
        ...
        for i, rxn_ in enumerate(open(self.raw_paths[self.file_idx], 'r')):
            ...
            for j, p in enumerate(products):
                ...
                gi_nodes, gi_edge_index, gi_edge_attr, atom_map = mol.rxn_to_graph_supernode(mol=p, atom_types=atom_types, bond_types=bond_types,
                                                                                             ...
                                                                                             with_formal_charge=self.with_formal_charge,
                                                                                             add_supernode_edges=False, get_atom_mapping=True,
                                                                                             canonicalize_molecule=self.canonicalize_molecule)
...
class DataModule(AbstractDataModule):
    ...
    def prepare_data(self, shuffle=True, seed=0) -> None:
        ...
        datasets = {'train': Dataset(stage='train', root=root_path, with_explicit_h=self.with_explicit_h, add_supernode_edges=self.add_supernode_edges,
                                     with_formal_charge=self.with_formal_charge, max_nodes_more_than_product=self.max_nodes_more_than_product, canonicalize_molecule=self.canonicalize_molecule),
                    'val': Dataset(stage='val', root=root_path, with_explicit_h=self.with_explicit_h, add_supernode_edges=self.add_supernode_edges,
                                   with_formal_charge=self.with_formal_charge, max_nodes_more_than_product=self.max_nodes_more_than_product, canonicalize_molecule=self.canonicalize_molecule),
                    'test': Dataset(stage='test', root=root_path, size_test_splits=self.cfg.test.size_test_splits, add_supernode_edges=self.add_supernode_edges,
