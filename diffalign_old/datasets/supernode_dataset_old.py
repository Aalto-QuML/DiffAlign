...
class Dataset(InMemoryDataset):
    def __init__(self, stage, root, size_test_splits=0, with_explicit_h=False, with_formal_charge=False, add_supernode_edges=True, seed=1):
        self.stage = stage
    ...
    def process(self):
        ...
        for i, rxn_ in enumerate(open(self.raw_paths[self.file_idx], 'r')):
            ...
            for j, r in enumerate(reactants):
                gi_nodes, gi_edge_index, gi_edge_attr = mol.rxn_to_graph_supernode(mol=r, atom_types=atom_types, 
...
class DataModule(AbstractDataModule):
    ...
    def prepare_data(self, shuffle=True, seed=0) -> None:
        ...
        datasets = {'train': Dataset(stage='train', root=root_path, seed=self.seed, add_supernode_edges=self.add_supernode_edges , with_explicit_h=self.with_explicit_h, 
                                     with_formal_charge=self.with_formal_charge),
                    'val': Dataset(stage='val', root=root_path, seed=self.seed, add_supernode_edges=self.add_supernode_edges, with_explicit_h=self.with_explicit_h, 
                                   with_formal_charge=self.with_formal_charge),
                    'test': Dataset(stage='test', root=root_path, seed=self.seed, size_test_splits=self.cfg.test.size_test_splits, add_supernode_edges=self.add_supernode_edges,
