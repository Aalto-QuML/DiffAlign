...
from diffalign_old.utils import retrobridge_data as utils
from diffalign_old.datasets.abstract_dataset_retrobridge import MolecularDataModule
...
class RetroBridgeDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        for i, reaction_smiles in enumerate(tqdm(table['reactants>reagents>production'].values)):
            ...
            try:
                ...
                r_x, r_edge_index, r_edge_attr = self.compute_graph(rmol, mapping, r_num_nodes)
                p_x, p_edge_index, p_edge_attr = self.compute_graph(pmol, mapping, r_num_nodes)
    ...
    def compute_graph(self, molecule, mapping, max_num_nodes):
        max_num_nodes = max(molecule.GetNumAtoms(), max_num_nodes)  # in case |reactants|-|product| > max_n_dummy_nodes
        type_idx = [len(self.types) - 1] * max_num_nodes
        for i, atom in enumerate(molecule.GetAtoms()):
            type_idx[mapping[atom.GetAtomMapNum()]] = self.types[atom.GetSymbol()]
        ...
        for bond in molecule.GetBonds():
            ...
            row += [start, end]
            col += [end, start]
            edge_type += 2 * [self.bonds[bond.GetBondType()] + 1]
...
class RetroBridgeDatasetInfos:
    ...
    def __init__(self, datamodule):
        ...
        self.scoring_input_dims = None
        self.two_track_scoring_input_dims = None
        self.size_input_dims = None
        self.remove_h = True
    ...
    def compute_input_output_dims(self, datamodule, extra_features, domain_features, use_context):
        ...
        self.scoring_input_dims = {
            'X': example_batch['x'].size(1) * 2 + ex_extra_feat.X.size(-1) + ex_extra_molecular_feat.X.size(-1),
            'E': example_batch['edge_attr'].size(1) * 2 + ex_extra_feat.E.size(-1) + ex_extra_molecular_feat.E.size(-1),
            'y': example_batch['y'].size(1) + ex_extra_feat.y.size(-1) + ex_extra_molecular_feat.y.size(-1),
        }
        ...
        self.two_track_scoring_input_dims = {
            'X': example_batch['x'].size(1) + ex_extra_feat.X.size(-1) + ex_extra_molecular_feat.X.size(-1),
            'E': example_batch['edge_attr'].size(1) + ex_extra_feat.E.size(-1) + ex_extra_molecular_feat.E.size(-1),
            'y': example_batch['y'].size(1) + ex_extra_feat.y.size(-1) + ex_extra_molecular_feat.y.size(-1),
        }

        self.size_input_dims = {
            # The last dimension (dummy node type) is not used in X for size prediction model
            'X': example_batch['x'].size(1) - 1 + ex_extra_feat.X.size(-1) + ex_extra_molecular_feat.X.size(-1),
            'E': example_batch['edge_attr'].size(1) + ex_extra_feat.E.size(-1) + ex_extra_molecular_feat.E.size(-1),
            'y': example_batch['y'].size(1) + ex_extra_feat.y.size(-1) + ex_extra_molecular_feat.y.size(-1),
        }
