...
from diffalign_old.utils import io_utils, mol
...
def process_reaction(i, matched_database_rxns_with_am, matched_database_rxns, sampled_rxns, cfg):
    ...
    if not matched_rxn is None: # This should not happen but can still happen due to way the code handles situatinos whern there's not enough dummy nodes -> but yeah anyways it's not going to be a ground-truth match anyways in this case
        matched_prod_mol = Chem.MolFromSmiles(matched_rxn_with_am.split('>>')[1])
        samples_with_chirality = []
        for j in range(len(samples)):
            prod_smi_without_am = matched_rxn.split('>>')[1]
...
def main(cfg: DictConfig):
    ...
    raw_data_path = "/u/38/rissans2/unix/Work/RetroDiffuser/data/uspto-50k-dummy15-stereo-charges-in-atom-types/raw/test.csv"
    old_samples_path = "/u/38/rissans2/unix/Work/RetroDiffuser/experiments/0h8u03an/eval_epoch60_steps100_resorted_0.9_cond4992_sampercond100_test_lam0.9_gam0.txt"
    ...
    # assert None not in gen_index_to_database_rxn, 'Some generated reactions could not be matched with database reactions.'
