...
from diffalign_old.diffusion.diffusion_abstract import DiscreteDenoisingDiffusion
from diffalign_old.utils import graph, mol
from diffalign_old.datasets import supernode_dataset_old
from diffalign_old.utils.diffusion.helpers import rxn_diagnostic_chains, rxn_vs_sample_plot
from diffalign_old.utils.diffusion import helpers
from diffalign_old.utils import io_utils
...
class DiscreteDenoisingDiffusionRxn(DiscreteDenoisingDiffusion):
    ...
    def product_smiles_to_reactant_smiles(self, product_smiles):
        ...
        # canonicalize
        # product_smiles = Chem.MolToSmiles(Chem.FromFromSmiles(product_smiles))
        ...
        if self.cfg.dataset.add_supernodes:
            data = data_utils.add_supernodes(self.cfg, data)
    ...
    def evaluate_from_artifact(self, reaction_data, actual_n_conditions, epoch=None, device=None, condition_range=None):
        ...
        # try:
        true_graph_data, sample_graph_data = io_utils.dense_from_pyg_file_data_for_reaction_list(self.cfg, reaction_data)
        # except:
        #     log.warning("problem!")
    ...
    def evaluate_from_artifact_new(self, reaction_data, epoch=None, device=None, condition_range=None):
        ...
        for i in range(len(reaction_data['gen'])):
            try:
                dense_data = io_utils.dense_from_pyg_file_data(self.cfg, reaction_data['true'][i])
                final_samples = io_utils.dense_from_pyg_file_data(self.cfg, reaction_data['gen'][i])
                ...
                scores_ = helpers.average_rxn_scores(scores_all, counts_of_samples)
