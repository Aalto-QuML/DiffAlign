...
class GraphTransformer(nn.Module):
    ...
    def __init__(self, n_layers: int, input_dims: dict, hidden_mlp_dims: dict, hidden_dims: dict,
        ...
        self.node_pe = PositionalEncoding(d_model=self.out_dim_X)
        self.edge_pe = PositionalEncoding(d_model=self.out_dim_E)

    def forward(self, X, E, y, node_mask, use_pe=False):
        # NOTE: we use positional encodings in multiple places in the forward pass
        # to 'refresh' the positional information in the embeddings as we pass through the layers

        if use_pe:
            node_pe = PositionalEncoding(d_model=X.shape[-1])
            edge_pe = PositionalEncoding(d_model=E.shape[-1])
            X = node_pe(X)
            E = edge_pe(E)
