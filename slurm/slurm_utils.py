...
def add_platform_specific_slurm_commands(fh, slurm_args):
    if slurm_args['platform'] == 'lumi':
        ...
        fh.writelines(f"export PYTHONUSERBASE=/projappl/{slurm_args['project']}/{slurm_args['venv_path']}\n")
    else:
...
def add_script_commands(script_args, slurm_args, fh=None):
    ...
    if slurm_args['platform'] == 'lumi':
        return add_script_commands_lumi(script_args, slurm_args, fh)
    elif slurm_args['platform'] == 'puhti':
        return add_script_commands_puhti(script_args, slurm_args, fh)
    else:
        raise ValueError(f"Platform {slurm_args['platform']} not supported")

def add_script_commands_puhti(script_args, slurm_args, fh=None):
    ...
    with open(job_file, 'w') as fj:
        ...
        # fj.writelines(f"export PYTHONUSERBASE=/projappl/{slurm_args['project']}/{slurm_args['venv_path']}\n")
...
def add_script_commands_lumi(script_args, slurm_args, fh=None):
    job_file = os.path.join(slurm_args['job_dir'], 
                            f"{slurm_args['job_name']}.sh")
    script_path = os.path.join(PROJECT_ROOT,
                                script_args['script_dir'], 
