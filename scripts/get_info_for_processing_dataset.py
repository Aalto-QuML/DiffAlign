...
def get_info_for_processing_dataset(cfg):
    ...
    # reactions_train = open(os.path.join(PROJECT_ROOT, 'data', cfg.dataset.data_dir,
    #                                     'raw', 'train.csv'), encoding='utf-8').readlines()
    reactions_train = pd.read_csv(os.path.join(PROJECT_ROOT, 'data', cfg.dataset.data_dir,
    ...
    for idx, reaction_smiles in enumerate(reactions_train):
        ...
        for molecule in reactants+products:
            ...
            for atom in mol.GetAtoms():
                ...
                atom_charges.append(get_formal_charge_as_str(atom.GetFormalCharge()))
    ...
    # process the lists
    atom_types = [a+'\n' for a in set(atom_types)]
    atom_types_charged = [a+'\n' for a in set(atom_types_charged)]
    atom_charges = [a+'\n' for a in set(atom_charges)]
    atom_chiral_tags = [a+'\n' for a in set(atom_chiral_tags)]
    bond_types = [a+'\n' for a in set(bond_types)]
    ...
    # save info
    os.makedirs(os.path.join(PROJECT_ROOT, 'data', cfg.dataset.data_dir, 'processed'), exist_ok=True)
    open(os.path.join(PROJECT_ROOT, 'data', cfg.dataset.data_dir,
