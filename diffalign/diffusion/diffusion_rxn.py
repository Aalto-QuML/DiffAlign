...
from diffalign.utils.diffusion.helpers import rxn_diagnostic_chains, rxn_vs_sample_plot
...
class DiscreteDenoisingDiffusionRxn(DiscreteDenoisingDiffusion):
    ...
    def eval_full_dataset(self, dataloader, epoch=0, inpaint_node_idx=None, inpaint_edge_idx=None, device=None): 
        raise NotImplementedError("eval_full_dataset has not been tested")
    ...
    def _compute_validity_scores(self, final_samples, true_rxn_smiles, n_samples):
        '''Compute validity scores for generated samples.

        Returns:
            (all_valid, atleastone_valid, all_valid_unfiltered, atleastone_valid_unfiltered)
        '''
        t0 = time.time()
        ...
        return all_valid, atleastone_valid, all_valid_unfiltered, atleastone_valid_unfiltered

    def _compute_matching_scores(self, true_data, final_samples, true_rxn_smiles, gen_rxn_smiles, unique_indices, bs, n_samples, n, device):
        '''Compute atom/SMILES matching scores and aggregate per batch.

        Returns:
            scores: dict with validity and matching entries.
        '''
        all_valid, atleastone_valid, all_valid_unfiltered, atleastone_valid_unfiltered = self._compute_validity_scores(
            final_samples, true_rxn_smiles, n_samples)
    ...
    def _compute_topk_scores(self, final_samples, gen_rxn_smiles, true_rxn_smiles, is_unique, counts, bs, n_samples, scores):
        '''Compute ELBO estimation, sorting, and top-k accuracy.

        Updates scores dict in-place with top-k entries.

        Returns:
            (scores, elbo_sorted_rxns, weighted_prob_sorted_rxns)
        '''
    ...
    def score_one_batch(self, final_samples, true_data, bs, n_samples, n, device):
        ...
        scores = self._compute_matching_scores(true_data, final_samples, true_rxn_smiles, gen_rxn_smiles,
                                               unique_indices, bs, n_samples, n, device)

        scores, elbo_sorted_rxns, weighted_prob_sorted_rxns = self._compute_topk_scores(
            final_samples, gen_rxn_smiles, true_rxn_smiles, is_unique, counts, bs, n_samples, scores)
