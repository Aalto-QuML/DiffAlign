import torch
from diffalign.utils.graph import PlaceHolder
import logging
...
def batched_scipy_eigh(matrices):
    """
    Compute eigenvalues and eigenvectors for a batch of matrices.
    
    :param matrices: numpy array of shape (batch_size, n, n)
    :return: tuple (eigenvalues, eigenvectors)
             eigenvalues: numpy array of shape (batch_size, n)
             eigenvectors: numpy array of shape (batch_size, n, n)
    """
    batch_size, n, _ = matrices.shape
    eigenvalues = np.zeros((batch_size, n))
    eigenvectors = np.zeros((batch_size, n, n))

    for i in range(batch_size):
        eigenvalues[i], eigenvectors[i] = scipy.linalg.eigh(matrices[i])
...
class EigenFeatures(nn.Module):
    ...
    def __call__(self, E, node_mask):
        ...
        if self.mode == 'eigenvalues':
            ...
            # eigvals, eigvectors = batched_scipy_eigh(L)
            # eigvals = torch.from_numpy(eigvals).type_as(A)
            # eigvectors = torch.from_numpy(eigvectors).type_as(A)
            eigvals = eigvals.type_as(A) / torch.sum(mask, dim=1, keepdim=True)
