...
from diffalign.model.transformer_model_with_y import PositionalEmbedding
from diffalign.model.noise_schedule import *
from diffalign.data import graph, model_utils, mol
from diffalign.model.ema_pytorch import EMA
...
class DiscreteDenoisingDiffusion(nn.Module):
    def __init__(self, cfg, dataset_infos, node_type_counts_unnormalized=None, 
        ...
        self.eps = 1e-6
        self.log_every_steps= cfg.training.log_every_steps
    ...
    def training_step(self, data, i, device):
        ...
        if node_count > self.cfg.training.max_nodes_for_training:
        ...
        if torch.cuda.is_available() and self.cfg.training.use_mixed_precision: #and self.model.training:
            if self.cfg.training.use_bfloat16:
        ...
        elif self.cfg.training.loss=='vb':
        ...
        elif self.cfg.training.loss=='vbce':
        ...
        if self.cfg.test.with_denoising:
            ...
            for i in reversed(range(len(t_steps))):
                ...
                if get_chains and (s_int%self.cfg.training.log_every_t==0 or s_int==self.T-1): 
                    ...
                    # pred_0_chain = pred.slice_by_idx(idx=self.cfg.training.chains_to_save)
                    ...
                    # prob_s_chain = prob_s.slice_by_idx(idx=self.cfg.training.chains_to_save)

                    # save the chain of the actual sample  
                    # sample = z_t.slice_by_idx(idx=self.cfg.training.chains_to_save)  
    ...
    def sample_one_batch(self, device=None, n_samples=None, data=None, get_chains=False, get_true_rxns=False, inpaint_node_idx=None, inpaint_edge_idx=None):
        ...
        for i in reversed(range(len(t_steps))):
            ...
            if self.cfg.diffusion.count_conditioning_gamma != 0:                
                pred, grad_log_p_y_x_t = helpers.grad_log_p_y_x_t_approx(self, z_t, self.cfg.diffusion.count_conditioning_a, self.cfg.diffusion.count_conditioning_b, self.cfg.diffusion.count_conditioning_gamma, self.cfg.dataset.atom_types.index('U'))
