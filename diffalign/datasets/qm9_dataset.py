...
from diffalign.datasets.abstract_dataset import MolecularDataModule, AbstractDatasetInfos, AbstractDataModule
...
from diffalign.utils.setup import DistributionNodes
...
class Dataset(InMemoryDataset):
    ...
    def __init__(self, stage, root, remove_h: bool, target_prop=None, transform=None, pre_transform=None, pre_filter=None):
    ...
    def process(self):
        ...
        print(f'self.split_paths[self.file_idx] {self.split_paths[self.file_idx]}\n')
        exit()
...
class DataModule(AbstractDataModule):
    ...
    def prepare_data(self, shuffle=True, seed=0) -> None:
        ...
        datasets = {'train': Dataset(stage='train', root=root_path, remove_h=self.cfg.dataset.remove_h,
                                        target_prop=target, transform=RemoveYTransform()),
                    'val': Dataset(stage='val', root=root_path, remove_h=self.cfg.dataset.remove_h,
                                      target_prop=target, transform=RemoveYTransform()),
                    'test': Dataset(stage='test', root=root_path, remove_h=self.cfg.dataset.remove_h,
                                       target_prop=target, transform=transform)}
        super().prepare_data(datasets, shuffle=shuffle, seed=seed)

class DatasetInfos:
    def __init__(self, datamodule, remove_h, recompute_info=False):
        self.remove_h = remove_h
...
def get_train_smiles(cfg, train_dataloader, dataset_infos, evaluate_dataset=False):
    ...
    if evaluate_dataset:
        ...
        for i, data in enumerate(train_dataloader):
            dense_data, node_mask = graph.to_dense(data)
