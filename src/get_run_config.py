...
def main(cfg: DictConfig):
    ...
    if cfg.general.wandb.mode=='online': run, cfg = setup.setup_wandb(cfg, job_type='training') 
    dir_ = os.path.join(parent_path, 'configs', 'old_runs')
    setup.mkdir_p(dir_)
    # note that run.id!=cfg.general.wandb.run_id because we are not resuming runs here
    # we want to use the id of the target run, given in cfg.general.wandb.run_id
