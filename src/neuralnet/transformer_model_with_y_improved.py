...
class GraphTransformerWithYImproved(nn.Module):
    """
    ...
    def forward(self, X, E, y, node_mask):
        ...
        return graph.PlaceHolder(X=X, E=E, y=y).mask(node_mask)
...
class GraphTransformerWithYAndPosEmb(nn.Module):
    ...
    def forward(self, X, E, y, node_mask):
        ...
        X = X + self.pos_emb(torch.arange(n).type_as(X).repeat(bs)).reshape(bs, n, self.pos_emb.dim)
